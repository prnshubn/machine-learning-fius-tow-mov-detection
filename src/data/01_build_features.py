...
def build_feature_dataset():
    ...
    if os.path.exists(output_filepath):
        # Get the latest modification time from all raw files
        latest_raw_mtime = max(os.path.getmtime(f) for f in csv_files)
        output_mtime = os.path.getmtime(output_filepath)
        
        if output_mtime > latest_raw_mtime:
            print(f"Step 1: '{output_filepath}' is up to date (Fast mtime check). Skipping.")
            return
        else:
            print(f"Step 1: Raw data has changed. Rebuilding feature dataset...")
    ...
    for filepath in csv_files:
        ...
        if len(parts) < 3 or parts[0] != 'signal':
            print(f"ERROR: File '{filename}' does not follow naming convention. Skipping.")
            continue
    ...
    if valid_files_count == 0:
        print("\nERROR: No valid files following the naming convention were found.")
        sys.exit(1)
